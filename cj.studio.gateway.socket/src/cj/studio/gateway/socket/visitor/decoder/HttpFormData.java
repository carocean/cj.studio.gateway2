...
public class HttpFormData {
	...
	* ------WebKitFormBoundaryFWSTsCnoOkljFCsb
	 ...
	 * </pre>
	 */
	public HttpFormData(byte[] b, String boundary) {
		this();
		HttpFormBoundary bd = new HttpFormBoundary(boundary);
	...
	public void load(byte[] b, String boundary) {
		HttpFormBoundary bd = new HttpFormBoundary(String.format("--%s", boundary));
		for (int i = 0; i < b.length; i++) {
			write(b[i], bd);
		}
	}

	public void add(HttpFieldData fd) {
		fields.add(fd);
	}

	public Iterator<HttpFieldData> iterator() {
		return fields.iterator();
	}

	public HttpFieldData get(int index) {
	 ...
	 * @param bd
	 */
	public synchronized void write(byte b, HttpFormBoundary bd) {
		int d = bd.write(b);
