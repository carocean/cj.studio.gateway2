...
class MicCommandFactory implements IMicCommandFactory, IServiceProvider {
    ...
    public void exeCommand(String cmdline, String user, Frame frame) throws CircuitException {
        ...
        if ("openports".equals(cmdName)) {
            IConfiguration config = (IConfiguration) parent.getService("$.config");
            List<String> openportsUrls = config.registry().getMic().openportsUrls;
            response.send(user, String.format("$openports{%s}", new Gson().toJson(openportsUrls)));
            return;
        }
