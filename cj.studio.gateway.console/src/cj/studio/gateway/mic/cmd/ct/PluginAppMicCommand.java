...
public class PluginAppMicCommand extends MicCommand {
	...
	public String cmdDesc() {
		return "安装插件程序包到应用目标。例：plugin website -d 插件目录名";
	}

	@Override
	public Options options() {
		Options options = new Options();
		Option c = new Option("d", "dir", true, "指定插件目录名");
		options.addOption(c);
		return options;
	}
	...
	public void doCommand(CommandLine line, String user, ISendResponse response, Frame frame,
		...
		String pluginDir=line.getOptionValue("d");
		if(StringUtil.isEmpty(pluginDir)) {
			sb.append("<li><span>缺少参数-d</span></li>");
			sb.append("</ul>");
			response.send(user, sb.toString());
			return;
		}
		...
		String dir = String.format("%s%sassemblies%s%s%splugins%s%s%s", gatewayHome, File.separator, File.separator, domain,
				File.separator,File.separator,pluginDir,File.separator);
